...
#elif defined(__GNUC__) && !defined(AS_NO_THREADS)

// ESP-IDF / GCC fallback for platforms that are not recognized by the
// stock AngelScript platform detection, e.g. ESP32-P4 RISC-V.
//
// AngelScript needs atomic increment/decrement for thread-safe reference
// counting whenever AS_POSIX_THREADS is enabled. GCC's __sync builtins
// provide full-barrier atomic operations and are supported by the
// riscv32-esp-elf toolchain used by ESP-IDF.
